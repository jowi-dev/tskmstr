...
fn hint_for(screen: Screen, show_run_detail: bool) -> &'static str {
    match screen {
        ...
        Screen::Board => {
            "h/l column  j/k move  Enter open  r refresh  o browser  O jira  f filter  A assign  p priority  a audit  s session  m manual  w work  b bots  v view run  L logs  V vdiff  F fix  R retro  ? help  q quit"
        }
