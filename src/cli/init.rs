...
pub fn run_init(
    ...
    let lane_scaffolds = lane_step(ctx, yes, &mut doc, &repo_config_path, prompter, out)?;
...
/// frontmatter.
///
/// Skipped under `yes`: a scripted run has no way to declare a model, and a
/// model string is the one thing the definition can't sensibly default. An
/// existing definition file is never overwritten — the name is still
/// recorded, but no scaffold is returned.
#[allow(clippy::too_many_arguments)]
fn offer_subagent(
    ctx: &InitContext,
    yes: bool,
    ...
    let model = ask_required(
        ...
        out,
        &format!(
            "Subagent model (the model {} runs delegated work as)",
            ctx.runner.display_name()
        ),
        "",
        "the subagent model",
    ...
    let scaffold = if def_path.exists() {
        ...
        Some((
            def_path,
            ctx.runner
                .agent_definition_template(&agent_name, Some(&model)),
        ))
    };
...
mod tests {
    ...
    fn setup_offer_declined_keeps_the_static_skeleton() {
        ...
        // true, subagent false, audit false, review-watch false, setup false.
        let mut prompter = FakePrompter::new()
            .with_confirm(true)
            .with_confirm(true)
            .with_confirm(true)
    ...
    fn setup_prompt_lists_missing_session_skills() {
        ...
        // false, audit true, review-watch false, setup true.
        let mut prompter = FakePrompter::new()
            .with_confirm(true)
            .with_confirm(true)
            .with_confirm(true)
